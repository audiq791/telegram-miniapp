...
export default function ProfileScreen() {
  ...
  const handleLogin = () => {
    setIsLoggedIn(true);
    console.log("Вход с номером:", phoneNumber);
  };

  // Заглушка для зарегистрированного пользователя
  if (isLoggedIn) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
      ...
      >
        ...
        <div className="max-w-md mx-auto px-4 pt-6">
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6">
            <p className="text-center text-zinc-600">Вы вошли как пользователь</p>
            <p className="text-center text-sm text-zinc-400 mt-2">+7 {getDisplayNumber()}</p>
            <button 
              onClick={() => setIsLoggedIn(false)}
              className="w-full mt-4 py-2 text-red-600 text-sm"
            >
              Выйти
            </button>
          </div>
